...
test.each([
  [1, 10, 'test abcd', 'test title'],
  [2, 2, 'Notability', 'img.url'],
])('MiscCard display id %d', (id, noteCount_, description_, title_) => {
  const initialState = {};
  ...
  const container = render(
    <BrowserRouter>
      <Provider store={store}>
        <MiscCard property={{ title: `${title_}`, description: `${description_}`, noteCount: `${noteCount_}` }} />
      </Provider>
    </BrowserRouter>,
  );
